<template>
  <div ref="quotation-form" class="quotation-form">
    <div class="quotation-form--state">
      ...
      <div
        @click="changeState(3, state)"
        :class="[
          'quotation-form--state--step',
          { active: state === 3, done: state > 3 },
        ]"
      >
        ...
        <div class="information">
          ...
          <span class="description">{{
            $t('pages.quotation.form.step_3')
          }}</span>
        </div>
      </div>
    </div>
    <transition name="slide">
      <Contact v-if="state === 1" :form="form" @submit="onContactSubmit" />
    </transition>
    <transition name="slide">
      <Project v-if="state === 2" :form="form" @submit="onProjectSubmit" />
    </transition>
    <transition name="slide">
      <SilosDimensions
        v-if="state === 3"
    ...
    async onSilosDimensionsSubmit(silosDimensions) {
      ...
      try {
        ...
        await this.create({
          attributes: {
            title:
              this.form.field_firstname +
              ' ' +
              this.form.field_lastname +
              ' (' +
              date.getDay() +
              '/' +
              date.getMonth() +
